...
.process.upd [`ALLOCATIONS]: {
	t: `id xkey select
			id, totalqty: qty, nearqty: 0, nearleaves: 0, farqty: 0, farleaves: 0, unit: .allocation.multiple * minsize, lowqty: .allocation.low * minsize
	...
	upd [`FARALLOCATIONS;	0 ! select id, qty: unit from t where canallocate, farleaves <= lowqty];
	upd [`NEARALLOCATIONS;	0 ! select id, qty: unit from t where canallocate, nearleaves <= lowqty];
